...
export class NewDialogComponent {
	...
	keys: { name: string; value: string | SongKey }[] = Object.entries(SongKey)
		.map(([name, value]) => ({ name, value }))
		.filter((v) => isNaN(Number(v.name)));
	modes: { name: string; value: string | SongMode }[] = Object.entries(SongMode)
		.map(([name, value]) => ({ name, value }))
		.filter((v) => isNaN(Number(v.name)));
	...
	onSelectedTabChange(event: MatTabChangeEvent) {
		...
		if (this.selectedSong == undefined) this.createSongDisabled = true;
	}
